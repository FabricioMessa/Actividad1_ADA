...
string add(string lhs, string rhs) { //recibe 2 cadenas
    int length = max(lhs.size(), rhs.size()); //determina el tamaño de las cadenas recibidas
    int carry = 0;
    int sum_col;  // sum of two digits in the same column
    string result;

    // usamos estos while para ver si ambas cadenas son de igual tamaño, si no desiguales añadimos ceros a la cadena 
    ...
    for (int i = length - 1; i >= 0; i--) {
        sum_col = (lhs[i] - '0') + (rhs[i] - '0') + carry; //(lhs[i] - '0') + (rhs[i] - '0') dentro de ASCII si restamos 0 con el caracter del digito colocado nos dará su número equivalente
        carry = sum_col / 10; //calcula el valor de acarreo para la siguiente suma
        result.insert(0, to_string(sum_col % 10)); // inserta el digito resultante en la posición inicial e la cadena
    }
